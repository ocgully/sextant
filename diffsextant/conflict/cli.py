...
from diffsextant.conflict.render import render_text
from diffsextant.conflict.types import ConflictFile, ConflictRegion


# ---------------------------------------------------------------------------
# `diffsextant conflict <file>`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# `diffsextant merge-driver %O %A %B %P`
# ---------------------------------------------------------------------------
...
def cmd_merge_driver(args) -> int:
    """Called by git as `diffsextant merge-driver %O %A %B %P`.
...
def cmd_register_merge_driver(args) -> int:
    ...
    if args.scope == "repo":
        ...
        if _SENTINEL_OPEN in existing:
            sys.stdout.write(
                "(.gitattributes already has a diffsextant merge-managed block — skipped)\n"
            )
        else:
            block = (
                f"\n{_SENTINEL_OPEN}\n"
            ...
            sys.stdout.write(f"wrote diffsextant merge-driver block to {attrs}\n")

    sys.stdout.write(
        f"registered `merge.sextant` in git config ({scope_flag}).\n"
        f"git will now route 3-way merges through `diffsextant merge-driver`.\n"
    )
    return 0


# ---------------------------------------------------------------------------
# parser registration — called from diffsextant/cli.py
# ---------------------------------------------------------------------------
...
def add_subparsers(sub: argparse._SubParsersAction) -> None:
    ...
    md = sub.add_parser(
        "merge-driver",
        help="git merge driver entrypoint — `diffsextant merge-driver %%O %%A %%B %%P`",
    )
