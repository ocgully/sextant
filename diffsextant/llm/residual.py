...
When `--llm` is passed to `diffsextant diff`, every op below the LOW
...
def cache_path_for(op, *, cwd: Optional[Path] = None) -> Path:
    ...
    return storage_dir(cwd) / "cache" / "llm" / f"{cache_key_for_op(op)}.json"
...
def run_residual(operations,
    ...
    for op in pending:
        ...
        cache_path = sd / "cache" / "llm" / f"{key}.json"
        # Cache hit?
