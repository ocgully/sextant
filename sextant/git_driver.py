...
And `sextant diff` gains the protocol bridge:
...
# `sextant diff` to parse positional args as git's 7-tuple.
DRIVER_COMMAND = "sextant diff --git-driver-mode --format text"
...
def uninstall(*, scope: str = "repo",
              cwd: Optional[Path] = None,
              stream=sys.stdout) -> int:
    """Remove a previously-installed sextant diff driver. Idempotent."""
    scope_obj = _scope(scope)
    cwd = (cwd or Path.cwd()).resolve()
    if scope_obj.touches_attrs:
        try:
...
def render_driver_invocation(inv: GitDriverInvocation,
    ...
    `sextant diff <ref1> <ref2>`, there's no git range here — git has
    ...
    """
    from sextant.ops.base import FileChange, Operation, OperationKind
    from sextant.parse import detect_language
    from sextant.tree_delta import DiffResult, _per_file_classifiers
